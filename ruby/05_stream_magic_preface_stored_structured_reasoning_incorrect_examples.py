...
class LocalModelAgent(Agent):
    ...
    def __init__(self, config: dict) -> None:
        ...
        self.rag_incorrect = RAG(config["rag_incorrect"])
    ...
    def update(self, correctness: bool) -> bool:
        ...
        if correctness:
            ...
            self.rag_incorrect.insert(key=question, value=chunk)
            return True

PROMPT_BASE = """
Provide your brief reasoning process within 100 words and final diagnosis base on the reasoning.
Respond in the following format:
[Key Symptoms]
[Relationship to Possible Diagnosis]
Final Diagnosis: <number>. <diagnosis>
"""
...
class ClassificationAgent(LocalModelAgent):
    ...
    def get_shot_template() -> str:
        prompt = f"""\
        {{question}}
        Reasoning and diagnosis: {{answer}}"""
        return strip_all_lines(prompt)
    ...
    def get_fewshot_template(
        ...
        Here are some Positive Examples (Correct Diagnosis).
        
        {{correct_shots_text}}

        Here are some Negative Examples (Incorrect Diagnosis) provided for learning purposes.
        These examples may contain reasoning with incorrect attributions or flawed logic. 
        Use them to identify common mistakes and avoid making similar errors in your own diagnosis.

        {{incorrect_shots_text}}
    ...
    def __call__(
        ...
        correct_shots = self.rag.retrieve(query=text, top_k=self.rag.top_k) if (self.rag.insert_acc > 0) else []
        incorrect_shots = self.rag_incorrect.retrieve(query=text, top_k=self.rag_incorrect.top_k) if (self.rag_incorrect.insert_acc > 0) else []
        correct_shots_len = len(correct_shots)
        incorrect_shots_len = len(incorrect_shots)
        if correct_shots_len or incorrect_shots_len:
            correct_shots_text = "\n\n\n".join(correct_shots).replace("\\", "\\\\")
            incorrect_shots_text = "\n\n\n".join(incorrect_shots).replace("\\", "\\\\")
            try:
                prompt = re.sub(pattern=r"\{correct_shots_text\}", repl=correct_shots_text, string=prompt_fewshot)
                prompt = re.sub(pattern=r"\{incorrect_shots_text\}", repl=incorrect_shots_text, string=prompt_fewshot)
        ...
        self.update_log_info(log_data={
            ...
            "num_shots": str(correct_shots_len+incorrect_shots_len),
            "input_pred": prompt,
